...
async def today_handler(message: Message):
    # group_id = cursorTim.execute("SELECT group FROM users WHERE id = "id" ")
...
async def store_handler(message: Message, group=None):
    if group in grouplist:
        ...
        cursorUsr.execute("INSERT INTO users VALUES(?,?,?)", randint(0, 9999999999), group, None)
        # вставка пользователя в таблицу(id, group, subscribe)
        # получается каждый раз, когда он будет "менять группу", он будет менять свой id... Пока не вижу как это исправить
